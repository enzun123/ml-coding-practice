...
# .replace(np.man, 'b') : 결측치를 문자열 바꾸기 함수를 통해 처리
...
# 예시) 인덱스 1의 listed_in 열값 : International TV Shows, TV Dramas, TV Mysteries
genres = netflix['listed_in'].str.split(', ', expand=True).stack().value_counts()
genres
...
netflix_age_country_unstack = netflix_age_country.groupby('age_group')['countru'].value_counts().unstack()
...
# 사용자 정의 컬러맵 만들기
cmap = plt.matplotlib.color.LinearSegmentedColormap.from_list('',[])